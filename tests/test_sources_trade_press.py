...
def parse_fn(html):
    ...
    return [
        {
            "title": "CaskML brings machine learning to whiskey distilling",
...
def test_parses_feed_into_company():
    src = TradePressSource(
        feed_urls=["https://trade.example/list"], parse_fn=parse_fn, today=date(2026, 7, 20)
    )
